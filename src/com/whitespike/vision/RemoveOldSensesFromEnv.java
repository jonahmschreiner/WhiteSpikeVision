...
public class RemoveOldSensesFromEnv {
			...
			for (int i = 0; i < sensesCopied.size(); i++) {
				...
				if (overlappingSenses.size() > 0 && !newSensesIn.contains(currSense) && flag2) {
					...
					for (int j = 0; j < currSense.blob.pixels.size(); j++) {
						...
						Color checkRGB = new Color (oldEnvIn.rawEnv.currentDisplay.getRGB(currPixel.position.x, currPixel.position.y));
	...
	/**
	 * Determines whether the two lists share a pixel.
	 * @param list1
	 * @param list2
	 * @return Whether the two lists share a pixel.
	 */
	...
	/**
	 * Removes the sense from the abstract env's sense list in java and the database (not currently used in this product).
	 * @param dbIdIn The id of the sense to remove in the database.
	 * @param envIn The env whose sense list contains the id to be removed
	 */
	...
	/**
	 * Find blobs of the same color when updating the env (honestly don't remember what uses this).
	 * @param pixelsToCheckQueue
	 * @param imageIn
	...
	/**
	 * Find all senses in list that overlap bounding boxes with currSense.
	 * @param sensesIn The senses to search through for those that overlap with currSense.
	 * @param currSense The sense that it finds overlapping senses for.
	 * @return The senses that overlapped with currSense.
	 */
	 ...
	 * @param newSense
	 * @param potOldSense
	 * @return Result of the two bounding boxes overlapping or not.
	 */
	...
	/**
	 * Determines if two bounding boxes' x values could overlap.
	 * @param newSense
	 * @param potOldSense
	 * @return The answer.
	 */
	...
	/**
	 * Determines if two bounding boxes' y values could overlap.
	 * @param newSense
	 * @param potOldSense
	 * @return The answer.
	 */
